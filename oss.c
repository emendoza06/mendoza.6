...
static char log_file[256] = "weighted.dat";
...
int main(int argc, char *argv[]) 
{
	...
	while((opt = getopt(argc, argv, "hl:dtm:")) != -1)
	{
		switch(opt)
		{
			case 'h':
				...
				printf("	-m number    : 0 is using non weighted, while 1 is using LRU weighted algorithm (default is 0).\n");
				exit(EXIT_SUCCESS);
			...
			case 'm':
				...
				if(scheme_choice == 0)
				{
					strncpy(log_file, "nonweighted.dat", 255);
				}
				break;
	...
	if(scheme_choice == 1)
	{
		fprintf(stderr, "Using weighted scheme.\n");
	}
	else
	{
		fprintf(stderr, "Using Least Recently Use (LRU) algorithm non-weighted scheme\n");
	}
